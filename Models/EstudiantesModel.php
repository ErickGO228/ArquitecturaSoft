...
class EstudiantesModel extends Query{
    ...
    public function insertarEstudiante($codigo, $dni, $nombre, $carrera)
    {
        $verificar = "SELECT * FROM estudiante WHERE codigo = '$codigo'";
        $existe = $this->select($verificar);
        if (empty($existe)) {
            $query = "INSERT INTO estudiante(codigo,dni,nombre,carrera) VALUES (?,?,?,?)";
            $datos = array($codigo, $dni, $nombre, $carrera);
    ...
    public function actualizarEstudiante($codigo, $dni, $nombre, $carrera, $id)
    {
        $query = "UPDATE estudiante SET codigo = ?, dni = ?, nombre = ?, carrera = ? WHERE id = ?";
        $datos = array($codigo, $dni, $nombre, $carrera, $id);
        $data = $this->save($query, $datos);
        if ($data == 1) {
            $res = "modificado";
        } else {
